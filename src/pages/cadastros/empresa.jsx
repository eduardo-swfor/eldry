...
import { IconePredio } from '../../components/icons'
import { useEffect, useState } from 'react'
import Edicao from '../../components/telas/empresa/Edicao'
import Pesquisa from '../../components/telas/empresa/Pesquisa'
...
export default function Empresa() {
  ...
  const excluiItem = itemParaExclusao => {
    excluirObj(endpoints.EMPRESA, itemParaExclusao._id).then(retorno => {
      if (!retorno){
        return
      }
  ...
  useEffect(()=>{
    listarColecao(endpoints.EMPRESA).then(setDadosPesquisa)
  ...
  async function gravar(vo) {
    gravarObj(endpoints.EMPRESA, vo).then(retorno =>{
      if (!retorno) {
        return
      }
  ...
  return (
      <Layout titulo='Cadastro de empresas' icone={IconePredio()}>
        { 
          edicao ?
            ...
            <Pesquisa 
              dados={dadosPesquisa}
              novo={()=>{
                setEdicao({nome: '', grupoEmpresa:{nome:''}})
              }}
              editar={obj =>{
                setEdicao(obj)
              }}

...
import { incluiOcorrencia } from './utils/ocorrencia-utils'
import Colecoes from '../../data/colecoes.json'
import { formataData, trocaStringParaDateDoObjeto } from '../../utils/DateUtils'
import { getFilesFromRequest, gravaArquivosRelacionados } from './utils/upload-utils'

const diretorio = 'ocorrencias'
...
apiRoute.post(async (req, res) => {    
    ...
    const ocorrencia = await incluiOcorrencia(
        objDatas, 
        email
    )
    ...
    await gravaArquivosRelacionados(
        arquivos, 
        ocorrencia._id.toString(), 
        email
    )

    return res.status(200).json(`
        A ocorrência foi criada com a sequência ${ocorrencia.sequencia} 
        e previsão de encerramento para ${formataData(ocorrencia.dataPrevisaoEncerramento)}
    `)
})
